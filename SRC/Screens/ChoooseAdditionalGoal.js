...
const ChoooseAdditionalGoal = () => {
  ...
  return (
    <View style={styles.mainScreen}>
      <CustomText style={styles.subText}>Let's Get To Know You Better!</CustomText>
      <CustomText style={styles.heading}>What Additional Goal Do You Have?</CustomText>
        <View>
            {goals.map((item,index)=>{
                return(
                    <TouchableOpacity style={[styles.goal, item.id ==1 && styles.activeGoal]} key={index}>
                            <CustomText style={styles.goalText}>{item.text}</CustomText>
                    </TouchableOpacity>
                )
            })}            
        </View>
        <CustomText style={styles.description} numberOfLines={2}>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel tortor eros. Fusce sit amet tempus eli.</CustomText>
        <CustomButton
            onPress={() => {
